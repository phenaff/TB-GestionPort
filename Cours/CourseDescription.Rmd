...
- le module commence par une video-conférence d'une vingtaine de minutes pour résumer le sujet et répondre aux questions,

- en groupe de 3 ou 4, les étudiants réalisent ensuite les travaux pratiques propres à chaque unité, sous forme de notebooks "Rmarkdown". Tous les documents necessaires se trouvent sur GitHub, dans le dépot public XYZ.

- après chaque journée (deux modules), le travail est restitué lors d'une visio-conférence, deux groupes à la fois, soit 6-8 étudiants et l'enseignant. Chaque groupe désigne un "ambassadeur" pour chaque visio-conférence, charge à elle ou lui de relayer les questions du groupe. 

# Evaluation

Chaque groupe choisit de rendre 4 TP parmis les 8 proposés, chaque TP comptant pour 25\% de la note, qui sera commune à tous les membres du groupe. Les TP sont à réaliser en notebook "Rmarkdown" [@Xie2019] et à rendre au format .pdf. "Rmarkdown" est une technologie très utile à maitriser, car elle permet de produire des analyses *reproduisibles*, avec une mise en page de grande qualité. La présentation et mise en page des documents devra donc être soignée, et sera prise en compte dans l'évaluation. Les TP sont à rendre 10 jours après le cours correspondant.

# Roles dans chaque groupe de travail

Comme indiqué plus haut, à l'occasion de chaque video conférence, chaque groupe désigne un "ambassadeur" chargé de relayer les commentaires ou questions du groupe. De plus, pour chaque TP donnant lieu à une évaluation, chaque groupe désigne un "maître du temps" chargé de s'assurer que le travail soit rendu en temps et en heure.

# Objectifs du cours

1. Aprofondir les propriétés statistiques des séries chronologiques financières.

2. Maitriser le modèle classique "moyenne-variance" de Markowitz[@Markowitz1952], et comprendre ses limites. Savoir le mettre en oeuvre et analyser les résultats. Apréhender comment le modèle de Black-Litterman répond à certaines limitations du modèle de Markowitz. 

3. Comprendre l'apport de l'approche factorielle en gestion de portefeuille.

4. Apréhender les nouvelles approches de gestion fondées sur le "risk budgeting".

5. Réaliser un rapide survol des méthodes de gestion de portefeuille obligataire: immunizaion et dédication.

# Logiciel

A chaque séance, on utilisera le logiciel R pour mettre immédiatement en pratique les concepts présentés. Ce logiciel est devenu un outil incontournable en finance quantitative, et en particulier en gestion de portefeuille. 

# Avant le premier cours

Avant la première séance, installez R, RStudio et vérifiez que votre installation est opérationelle en éxécutant les deux scripts disponibles sur GitHub.
...
- Estimation de la correlation entre series.

## Module 2 (10/2): La théorie classique [@Markowitz1952, @Treynor1973]

On considère ici le travail d'Harry Markowitz, qui établit les fondements de la gestion quantitative. On évoquera également le modèle de Treynor-Black, qui est à l'origine de la distinction  "gestion active/gestion passive" en gestion de portefeuille.

TP 2:

- Construction d'une frontière efficiente.
- Construction d'un portefeuille optimal moyenne/variance.
- Estimation de $\alpha, \beta$ et construction d'un portefeuille selon Treynor & Black.

## Module 3 (9/3): Gestion de portefeuille et risque de modélisation. [@Boyle2012; @He2005]
...
- Suite de la séance précedente: modèle M/V et Black-Litterman.
